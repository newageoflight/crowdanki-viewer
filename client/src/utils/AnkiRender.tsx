...
export const RenderAnkiTemplateAsObjects = (note: NoteInterface, model: NoteModel, template: CardTemplateInterface): Array<TemplateComponents> => {
    ...
    if (model.type === 1) {
        ...
        return range(qTemplates.length).map((i) => {
            return {qHTML: qTemplates[i], aHTML: aTemplates[i], style: model.css}
        }
        )
    } else {
        return [{qHTML: Mustache.render(qTemplate, templateFields),
